export class ApplicationService {
  ...
  async getApplications(status = null) {
    try {
      ...
      if (error) return { success: false, error: 'Failed to fetch applications.' };
  ...
  async updateApplicationStatus(id, status, admin_note = null) {
    try {
      ...
      if (error || !data) return { success: false, error: 'Failed to update application.' };
      return { success: true, data };
    } catch (err) {
      return { success: false, error: err.message };
    }
  ...
  async checkApplicationStatus(aternos_username) {
    try {
      ...
      const { data: app, error } = await this.repo.findByUsername(normalized);

      if (error) return { success: false, error: 'Failed to check application status.' };
      if (!app) return { success: false, error: 'No application found for this username.' };
