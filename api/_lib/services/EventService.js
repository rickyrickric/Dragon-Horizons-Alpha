...
export class EventService {
  ...
  async createEvent(payload) {
    try {
      ...
      if (!['patch', 'world_reset', 'mystery'].includes(payload.event_type)) {
        return { success: false, error: 'Invalid event type. Must be patch, world_reset, or mystery' };
      }
      ...
      if (eventDate <= new Date()) {
        return { success: false, error: 'Event time must be in the future' };
      }
      ...
      console.error('EventService.createEvent exception:', err);
      return { success: false, error: err.message };
    }
  }
  ...
  async updateEvent(id, payload) {
    try {
      ...
      if (payload.event_time !== undefined) {
        ...
        updates.event_time = eventDate.toISOString();
      }
      ...
      console.error('EventService.updateEvent exception:', err);
      return { success: false, error: err.message };
    }
  }
  ...
  async deleteEvent(id) {
    try {
      ...
      console.error('EventService.deleteEvent exception:', err);
      return { success: false, error: err.message };
    }
  }
}
