...
export class SupabaseEventRepository extends EventRepository {
  ...
  async getById(id) { const checkError = this._checkSupabase(); if (checkError) return checkError; const { data, error } = await supabase.from('events').select('*').eq('id', id).single(); return { data, error }; }

  async create(payload) { const checkError = this._checkSupabase(); if (checkError) return checkError; const { data, error } = await supabase.from('events').insert({ event_type: payload.event_type, title: payload.title || null, description: payload.description, event_time: payload.event_time, duration: payload.duration || '7-Day', is_active: true }).select().single(); return { data, error }; }
